...
def train(
    ...
    for i, samples in enumerate(progress):
        ...
        valid_losses, all_stats, should_stop = validate_and_save(
            cfg, trainer, task, epoch_itr, valid_subsets, end_of_epoch
        )
        if all_stats is not None:
            save_list.append(all_stats)
        if should_stop:
            break
    ...
    with open(cfg.model.result_path, 'w') as fp:
        fp.write(json.dumps(save_list, indent=1))
...
def validate_and_save(
    ...
    all_stats = None
    if do_validate:
        valid_losses, all_stats = validate(cfg, trainer, task, epoch_itr, valid_subsets)
    ...
    return valid_losses, all_stats, should_stop
